...
Ext.define('Ext.pivot.plugin.configurator.DropZone', {
    ...
    getTopIndicator: function() {
        if (!this.topIndicator) {
            ...
            this.self.prototype.indicatorXOffset =
                Math.floor((this.topIndicator.dom.offsetWidth + 1) / 2);
        }
    ...
    onNodeOver: function(node, dragZone, e, data) {
        ...
        if (target && target.header && dragColumn) {
            doPosition = doPosition &&
                        dragColumn.getField().getSettings().isAllowed(target.header) &&
                        me.panel.isAllowed(target.header, dragColumn);
        }
