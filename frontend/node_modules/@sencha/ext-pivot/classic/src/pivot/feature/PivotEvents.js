...
Ext.define('Ext.pivot.feature.PivotEvents', {
    ...
    onCellEvent: function(view, tdCell, e) {
        ...
        if (ret !== false && e.type === 'click' &&
            Ext.fly(tdCell).hasCls(me.groupHeaderCollapsibleCls)) {
            if (leftItem.expanded) {
                leftItem.collapse();
                me.view.focusNode(
                    matrix.viewLayoutType === 'tabular'
                        ? me.dataSource.getTabularGroupRecord(leftItem)
                        : leftItem.records.collapsed
                );
            }
            else {
                leftItem.expand();
