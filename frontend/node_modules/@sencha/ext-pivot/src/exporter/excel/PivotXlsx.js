...
Ext.define('Ext.exporter.excel.PivotXlsx', {
    ...
    getContent: function() {
        ...
        if (!matrix || !store || !store.isStore || store.isDestroyed) {
            //<debug>
            Ext.raise('No pivot matrix provided to the exporter or there is no store defined' +
                      'on the matrix');
            //</debug>
        ...
        if (matrix && !matrix.calculateAsExcel) {
            Ext.raise('The pivot table calculations are different than what Excel does. ' +
                      'Check "calculateAsExcel" config on the pivot matrix!');
        }
        //</debug>
        ...
        for (i = 0; i < len; i++) {
            if (layout === 'compact') {
                Ext.Array.insert(rows[i].cells, 0, [{
                    value: i === len - 1
                        ? this.rowLabelsText
                        : (i === 0 && countAgg === 1 ? matrix.aggregate.items[0].excelName : null)
                }]);
            }
            else {
    ...
    generatePivotBodyTabularItem: function(params, item) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (i === 0) {
                rowItem.x.push(
                    Ext.Array.indexOf(uniqueValues[group.dimension.dataIndex], group.value)
                );
    ...
    setupUniqueAggregateNames: function(fields) {
        ...
        for (i = 0; i < len; i++) {
            ...
            for (j = 0; j < length; j++) {
                if (String(temp).toLowerCase() === String(fields[j]).toLowerCase() ||
