...
Ext.define('Ext.pivot.matrix.Local', {
    ...
    updateGrandTotalsToPivotStore: function() {
        ...
        if (me.totals.length === totals.length) {
            for (i = 0; i < me.totals.length; i++) {
                if (Ext.isObject(totals[i]) && Ext.isObject(totals[i].values) &&
                    (me.totals[i].record instanceof Ext.data.Model)) {
    ...
    updateRecordToPivotStore: function(item) {
        ...
        if (!item.children) {
            ...
            if (item.records) {
                dataIndex =
                    (me.viewLayoutType === 'compact' ? me.compactViewKey : item.dimensionId);
                data = me.preparePivotStoreRecordData(item);
                delete(data.id);
