...
Ext.define('Ext.pivot.axis.Local', {
    ...
    processRecord: function(record) {
        ...
        for (i = 0; i < dimCount; i++) {
            ...
            groupValue = Ext.callback(
                dimension.groupFn, dimension.scope || 'self.controller', [record], 0, me.matrix.cmp
            );
    ...
    filterTree: function() {
        ...
        for (i = 0; i < length; i++) {
            hasFilters = hasFilters ||
                        (me.dimensions.items[i].filter instanceof Ext.pivot.filter.Value);
        }
